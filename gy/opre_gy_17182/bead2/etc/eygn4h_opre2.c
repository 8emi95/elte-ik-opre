...
//	printf("elinsul a 2. beadando\n");
  ...
  //  printf("signal utanra eljutottam1\n");
	pid_t pid1, pid2;
	pid1 = fork();
	...
	if(pid1 == 0) //idomar
    {
		...
		//kill(pid2,SIGTERM); siguser kell 
		pause();
		...
		if(pid2 == 0) //madar
        {
            pause();//itt se paussal várunk
			
			printf("madárka elindult\n");
			time_t to;
			srand(time(&to));
				r = rand() % 5 + 1;
		//	kill(pid1,SIGTERM); sigusr kell
		}
		else { // parent
			
			pause(); // nem paussal várunk, valami manualt mondott
			...
			close(pipe1[0]);	
			write(pipe1[1],&clients[count] , sizeof(clients[count]));
			count++;
        //   waitpid(pid1, &status, 0);
		   int rand2;
			close(pipe1[1]);
			
			read(pipe2[0],&rand2 ,sizeof(rand2));
			close(pipe2[1]);
			printf("Company - I read the time!\n");
			fflush(NULL);
			
			printf("|%s|%s|%s|%s|%d| \n",actual.name,actual.adress,actual.email,actual.telNum,actual.priority,random);
