...
void szemafor_muvelet(int semid, int op){
    ...
    muvelet.sem_num = 0; // Which semaphore?
    muvelet.sem_op  = op; // op=1 up, op=-1 down // Operation
    muvelet.sem_flg = 0; // Flag - waits for another semaphore
...
int main (int argc,char* argv[]) {
    ...
    kulcs=ftok(argv[0],1);
    sh_mem_id=shmget(kulcs,MEMSIZE,IPC_CREAT|S_IRUSR|S_IWUSR); // instead of "kulcs": IPC_CREAT (cannot be accessed from the outside)
