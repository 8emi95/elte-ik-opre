...
// Ha két különálló folyamatot akarunk összepasszintani: 
// Második elküldi az elsõnek a PID-jét (signal, named pipe segítségével)
sem_t* szemafor_letrehozas(char*nev,int szemafor_ertek)
{
    sem_t *semid=sem_open(nev,O_CREAT,S_IRUSR|S_IWUSR,szemafor_ertek ); // nev = pathname
	if (semid==SEM_FAILED)
		perror("sem_open");
