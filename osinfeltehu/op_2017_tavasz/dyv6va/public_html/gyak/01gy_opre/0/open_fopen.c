...
{
	printf("\n**********\nUsing open - binary \n****************\n");
	int fileStatus = open(fname, O_RDONLY);
	if (fileStatus < 0)
	{
	...
	while (read(fileStatus, &c, sizeof(c))) //use write for writing
	{
		printf("%c", c); //we prints out the content of the file on the screen
	}
	printf("\n");
	lseek(fileStatus, 0, SEEK_SET); //position to the first character, SEEK_SET,SEEK_CUR,SEEK_END
	read(fileStatus, &c, sizeof(c));
