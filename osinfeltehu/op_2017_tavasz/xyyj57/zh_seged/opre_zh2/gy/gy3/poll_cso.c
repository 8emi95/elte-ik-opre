...
//utemezest csak akkor kap a process, ha mar kerul valami a csovezetekbe
int main(){
 ...
 struct pollfd poll_fds[5]; // poll file descriptor array 5 csovezetekunk lesz amit figyelunk
 ...
 int result=poll(poll_fds,1,1000); // addig nem kell a futasi jog, amig nem erkezik adat, (fajlleirok tombje, tenyleges adatszam, timer(1sec))
 ...
 else 
 {
	if (result>0)
	{
	if (poll_fds[0].revents & POLLIN) // ide nem jut a vezerles
		{
