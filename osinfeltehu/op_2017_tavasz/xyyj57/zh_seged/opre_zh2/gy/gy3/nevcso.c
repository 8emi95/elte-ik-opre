...
//ha nem csak a forkolt folyamatok altal szeretnenk hasznalni, hanem kulso processz altal
int main(int argc, char* argv[])
{
    int pid,fd;
    printf("Fifo start!\n");
    int fid=mkfifo("fifo.ftc", S_IRUSR|S_IWUSR ); // creating named pipe file, írásjogok
	...
	close(fd);
	// remove fifo.ftc
	unlink("fifo.ftc"); //cso torlese, a program leallitasa utan is el, ezert kell torolni -  ls -al f* csovek listazasa torles : rm fifp.ftc
        }
	else // child
	{
