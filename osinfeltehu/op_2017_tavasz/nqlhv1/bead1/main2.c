...
void csinaljGyereket(char* csonev){	
	...
	usleep((aktualisGyerek * 1000 + (rand() % 1000)) * 50);
	...
	int pipe = open(csonev,O_RDWR | O_NONBLOCK);
...
int main(){
	...
	for(i = 0; i < gyerekSzam; ++i)
	{
	    ...
	    strcat(tmp,buffer);
	    unlink(tmp);
	    cso[i] = mkfifo(tmp, S_IRUSR|S_IWUSR );
	    if ( cso[i] == -1)
	    {
	...
	while (keszGyerek != gyerekSzam) {
		///printf("g %d --> k %d\n", gyerekSzam, keszGyerek);
	}
	
	printf("\n\n Megerkezett az osszes gyerek \n\n");
