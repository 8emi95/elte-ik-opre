...
void szemafor_muvelet(int semid, int op) {
    ...
    muvelet.sem_num = 0;  // hangyadik szemafor
    muvelet.sem_op  = op; // op=1 up, op=-1 down 
    muvelet.sem_flg = 0;  // 0 esetén a normál értelemben vett szemafor működést produkálja
...
//
	...
	printf("Gyerek: Indul a szemafor down!\n");
       ...
       szemafor_muvelet(semid, 1); // up // itt már a feladat szempontjából nincs értelme felemelni, de még is fel kell, mert addíg nem törlődhet a szemafor, amíg le van zárva. Zárt szemafor olyan, mintha még védeni akarna valamit.
