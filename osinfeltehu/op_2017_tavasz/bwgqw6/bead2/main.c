...
static struct result randomResults[20];
static struct Question questions[100];
static int qCounter = 0;

struct Question* selectRandomQuestions() {
	...
	for (i = 0; i < 3; ++i) {
		...
		int already = 0;
		srand(time(NULL));
		int r = rand() % qCounter; // modulo array size to make sure we dont overindex
		printf("RANDOM INDEX: %d",r);
	}
	/*
	while(i != 3) {
		printf("QCounter: %d",qCounter);
	...
	for (i = 0; i < 3; ++i) {
		printf("%d\n",selectedIndexes[i]);
		selectedQuestions[i] = questions[selectedIndexes[i]];
	}
	struct Question* q = selectedQuestions;
	return q;
	*/
}
struct result* getRandomResultsFromCity(char* cityName, struct Question* selectedQuestions) {
	printf("Getting random results in city %s...\n", cityName);
	struct Question* q = selectRandomQuestions();
	int i = 0;
...
int main(int argc, char** argv) { 
	...
	do {
		...
		switch(choice) {
			...
			case 4:
		    fp = saveSheet(questionPointer);
			temp_questionPointer = getQuestionsFromSheet(fp);
			printf("City name?");
			scanf("%s",&cityName);
			getRandomResultsFromCity(cityName, temp_questionPointer);
			break;
