...
int main(){
  ...
  sigaction(SIGTERM,&sigact,NULL); // beállíthatjuk, hogy melyik handler fusson lem és blokkolni is lehet vele
  ...
  {
    sigset_t sigset;
    sigfillset(&sigset); //itt az összes jelzést blokkoljuk
    sigdelset(&sigset,SIGTERM); // itt pedig feloldjuk azt ami nem akarunk blokkolni
    sigsuspend(&sigset); //várjon egy jelzésre, de csak miután kapott egy nem blokkolt jelzést
  ...
  else 
  {
    ...
    sleep(3);
    kill(getppid(),SIGUSR1); //itt ez blokkolva van, mert csak a SIGTERMET oldottuk fel
    printf("Waits 3 seconds, then send a SIGTERM %i signal (it is waited for by suspend)\n",SIGTERM);
    sleep(3);
    kill(getppid(),SIGTERM); // ez már fogadva lesz, mert ez nem volt blokkolva, feloldódik a blokkolás, majd a SIGUSR1
    //is megérkezik ugye
