 ...
 */
 ...
 myStruct.number = 2;
 strcpy(myStruct.string, "Peter"); // az argv[1] is lehetne a string erteke

 int f1, g1;

 g1=open("testText.txt", O_WRONLY|O_CREAT|O_TRUNC, S_IRUSR|S_IWUSR);
 write(g1, &myStruct, sizeof(myStruct));
 f1 = open("testText.txt", O_RDONLY);
 read(f1, &ms2, sizeof(ms2));
 printf("Szam: %d\nSzoveg: %s\n", ms2.number, ms2.string);
