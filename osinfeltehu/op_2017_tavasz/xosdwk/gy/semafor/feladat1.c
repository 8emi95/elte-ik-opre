...
int main (int argc,char* argv[]) {
    ...
    if(child>0){    
        ...
        sleep(1);            // child waits during sleep
        strcpy(s,buffer);
        ...
        if ( child2>0 ) {
            ...
            char * buffer = strcat(  s , "os nachos\n");
            printf("Gyerek, down rendben, hozzafuzte: %s",s);  
            szemafor_muvelet(semid,1); // up      
            // end of critical section  
            shmdt(s);
        }
        else if ( child2 == 0 ) {
            printf("gyerek2: downol\n");
            szemafor_muvelet(semid,-1); // down, wait if necessary
            printf("gyerek2: olvasta %s",s);
        }
    }
