...
int readAnsNumber(unsigned int max) {
    ...
    while (ansNumber == 0 || ansNumber > max) {
        if (tries > 0) {
            printf("Egy 1-%i kozotti egesz szamot varok!\n", max);
        }
...
Kerdoiv* getLastNode(Kerdoiv* kerdo) {
    Kerdoiv* p = kerdo->head;
    unsigned int i;
...
int menuAddAnswer(Kerdoiv* kerdo) {
    printf("\nMi legyen a valasz szovege?\t");
    char buffer[100];
    readString(buffer, sizeof(buffer));
    return addAnswer(kerdo, buffer);
}
 
int menuAddQuestion(Kerdesek* kerd) {
    printf("Mi legyen az uj kerdes szovege?\t");
    char qBuffer[100];
 
    readString(qBuffer, 100);
    addQuestion(kerd, qBuffer);
...
int menuDeleteQuestion(Kerdesek* kerd) {
    if (kerd->size == 0) {
        printf("A kerdoiv meg ures!\n");
        return 0;
    }
    ...
    if (deleteQuestion(kerd, atoi(buffer)) == 0)
        return 0;
    else {
...
int menuModifyAnswer(Kerdoiv* kerdo, unsigned int ansNumber) {
    printf("Mire szeretne modositani a kerdest?\t");
    char buffer[100];
    readString(buffer, sizeof(buffer));
    return modifyAnswer(kerdo, ansNumber, buffer);
}
...
int menuEditAnswer(Kerdoiv* kerdo, unsigned int nodeNumber) {
    ...
    Kerdoiv* currentNode = kerdo->head;
    unsigned int i;
    for (i = 1; i < nodeNumber; i++)
...
int menuModifyQuestion(Kerdesek* kerd, unsigned int nodeNumber) {
    printf("Mire szeretne modositani a kerdest?\t");
    char buffer[100];
   
    readString(buffer, sizeof(buffer));
    return modifyQuestion(kerd, nodeNumber, buffer);
}
 
int menuModifyQuestions(Kerdesek* kerd) {
    if (kerd->head == NULL) {
        printf("A kerdoiv meg ures!\n");
        return -1;
    }
    ...
    if (changeQuestion == 'i')
        returnVal = menuModifyQuestion(kerd, nodeNumber);
    ...
    if (returnVal == 0) {
        ...
        while (editAnswers == 'i') {
            ...
            if (editAnswers == 'i')
                returnVal = menuEditAnswer(kerd, nodeNumber);
        }
    }
...
void delQuestions(Kerdesek* kerd) {
    Kerdoiv* current = kerd->head;
    while (current != NULL) {
        Kerdoiv* next = current->next;
        free(current);
        current = next;
    }
    kerd->head = 0;
...
int main() {
    kerd fociKerdesek;
    Kerdesek* pfociKerdesek = &fociKerdesek;
