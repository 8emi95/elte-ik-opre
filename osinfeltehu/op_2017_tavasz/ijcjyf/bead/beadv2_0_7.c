...
fflush(stdin);
 ...
 printf("\n");
 	printf("Bejelentkezett---------------------------------- %s", argv[i+1]);
  printf("\n");
 }
	   ...
	   {		   
		   ...
		   char varos[100];
				...
				kill(getppid(),SIGTERM); 				
					close(pipeKBiz[1]);
					close(pipeMLSZ[0]);
			for (inedxx ; inedxx <varosokSzama; inedxx++){
		...
		//	printf("A kerdezobiztos megerkezett %s -be ! \n",varos );
			...
			//	read(pipeKBiz[0],&varos,sizeof(varos));
				//printf("A kerdezobiztos %s-ben van jeeee \n", varos);
				...
				printf("%s \n ", kapottKerdes3.kerdes);
				 ...
				 kapottKerdes1.valaszokSzama=0;
				 kapottKerdes2.valaszokSzama=0;
				 kapottKerdes3.valaszokSzama=0;
