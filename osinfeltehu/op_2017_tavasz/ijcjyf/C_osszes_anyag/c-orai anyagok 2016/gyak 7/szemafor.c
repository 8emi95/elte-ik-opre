...
int szemafor_letrehozas(const char* pathname,int szemafor_ertek){
    ...
    if((semid=semget(kulcs,1,IPC_CREAT|S_IRUSR|S_IWUSR ))<0) //1es: a szemafor csalad (tomb) egy tagu legyen
	perror("semget");
    // semget 2. parameter is the number of semaphores   
    if(semctl(semid,0,SETVAL,szemafor_ertek)<0)    //0= first semaphores CTL fuggveny szemafor erteket be kell allitani 0-zart
        perror("semctl");
...
void szemafor_muvelet(int semid, int op){
    ...
    muvelet.sem_num = 0;  //a csaladnak hanyadik tagjat akarom piszkalni
    muvelet.sem_op  = op; // mit akarok vele op=1 up, op=-1 down 
    muvelet.sem_flg = 0;  //ne lehessen lecsukni ha zart allapotban van
...
//
    semid = szemafor_letrehozas(argv[0],0); // sem state is down!!! 0as ertek le van zarva
//
    child = fork();
    if(child>0){    
