...
{
    sem_t *semid=sem_open(nev,O_CREAT,S_IRUSR|S_IWUSR,szemafor_ertek );//posixnél mindig a fajlnev azonositto /el kezdodik
	if (semid==SEM_FAILED)
	perror("sem_open");
...
int main (int argc,char* argv[]) {
	...
	shmdt(s);	// release shared memory	
       	wait(NULL);       
       	szemafor_torles(sem_nev); //utana mindent torlunk ha vegzett
