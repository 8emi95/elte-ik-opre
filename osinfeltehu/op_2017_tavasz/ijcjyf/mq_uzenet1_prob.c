...
int main (int argc, char* argv[]) { 
	...
	mq1=mq_open(mqname, O_CREAT|O_RDWR,0600,&attr);//létrehoz
     if ( mq1 < 0 ) { 
	...
	uzenet[13]=65; uzenet[14]=65; uzenet[15]=65;
	  int db=mq_send(mq1,uzenet,17, 4 );  //Parent sends a message.   paraméterek: uzenet leiro, uzenet tartalma, mérete, és prioritás
	  //sleep(1); // ha ez nincs a fogad be is olvassa!!!!
          mq_send(mq1,"Hajra Debrecen!\n",20, 5);// 4es prioritással megy elküldésre
	...
	//strcpy(buffer,"                  ");
        ...
        int db=mq_receive(mq1,buffer, 54,NULL);//papiron!
