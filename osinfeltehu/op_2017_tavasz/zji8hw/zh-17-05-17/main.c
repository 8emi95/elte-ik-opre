...
{
	printf("%d (Utas) Taxis jelzest kuldott!\n", getpid());
}

void handler2(int signumber, siginfo_t *info, void *nonused)
{
	int ar = info->si_value.sival_int * 300 + 700;
	printf("%d (Taxis) Utas kuldte a tavolsagot, az ar ez alapjan: %d .. %d\n", getpid(), ar, info->si_pid);
