...
int szemafor_letrehozas(const char *pathname, int szemafor_ertek)
{
    ...
    if ((semid = semget(kulcs, 1, IPC_CREAT | S_IRUSR | S_IWUSR)) < 0)  // semget 2. parameter is the number of semaphores
        perror("semget");
...
void szemafor_muvelet(int semid, int op)
{
    ...
    if (semop(semid, &muvelet, 1) < 0) // number of sem. operations: 1
        perror("semop");
}
