...
void handler(int signumber) {
    switch (signumber) {
	case SIGUSR1:
		printf("The received signal is %i \n", SIGUSR1);
		break;
	case SIGTERM:
		printf("We waited for this: SIGTERM %i \n", SIGTERM);
		break;
	}
...
// output:
// Waits 3 seconds, then send a SIGUSR 10 signal (it is not waited for by suspend) - so the suspend continues waiting
// Waits 3 seconds, then send a SIGTERM 15 signal (it is waited for by suspend)
// Child process ended
