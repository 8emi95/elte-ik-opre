...
void printMenu() {
	...
	printf("\t\t[5] A kerdessor veglegesitese es a konzultacio elkezdese\n");
	printf("\t\t[6] Kilepes\n");
}
...
int  printNodeToScreenWithResults(SurveyNode* node, int resultRow[ANSWER_MAX_COUNT]) {
	if (node == NULL)
		return -1;
...
int startVotingProcess(Survey* survey) {
	...
	if (child > 0) {
		...
		for (i = 0; i < 3; i++) {
			SurveyNode* p = &questionsToChild[i];
			printNodeToScreenWithResults(p, voteResultsFromChild[i]);
		}
	}
	else {
...
int main() {
	...
	while (running) {
		...
		case 4:
			...
			break;
		case 5:
			if (finalizeAndSaveSurvey(pFootballSurvey) == 0)
				running = 0;
			else
				printf("Legalabb 3 kerdes kell a konzultacio elkezdesehez!\n");
			break;
		case 6:
			running = 0;
			break;
