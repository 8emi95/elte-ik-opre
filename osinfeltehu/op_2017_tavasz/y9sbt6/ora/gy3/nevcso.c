...
int main(int argc, char* argv[])
{
    int pid,fd;
    printf("Fifo start!\n");
    int fid=mkfifo("fifo.ftc", S_IRUSR|S_IWUSR ); // creating named pipe file and rights who can use it
	...
	close(fd);
	// remove fifo.ftc
	unlink("fifo.ftc"); // töröljük ha ténylegesen meg akarunk tőle szabadulni és, hogy más ne férhessen hozzá
    }
    else // child
    {
