...
int main(){
  ...
  {
    sigset_t sigset;
    sigfillset(&sigset); // ezekre a jelzésekre ne reagáljon
    sigdelset(&sigset,SIGTERM); // teljes jelzéshalmazból kiveszem a sigtermet, azaz csak SIGTERMRE fog reagálni
    sigsuspend(&sigset); // jelzésre fog várni, de meg lehet neki mondani, hogy milyen jelzésre blokkoljon
