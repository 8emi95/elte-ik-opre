...
int szemafor_letrehozas(const char* pathname,int szemafor_ertek){
    ...
    // set the initial state of sempaphores -- here we create them in locked state
...
void szemafor_muvelet(int semid, int op){
    ...
    muvelet.sem_flg = 0;  // with 0 it always blocks, otherwise the caller can continue the computation
...
int main (int argc,char* argv[]) {
    ...
    kulcs=ftok(argv[0],1);
	// instead if using the ftok() function
	// we could use the constant value of IPC_CREAT 
	// this will create the shared object locally
	// so only internal programs can use it
