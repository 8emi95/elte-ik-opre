...
int sem_create(key_t key) // to create a semaphore
{
    int semaphore = semget(key, 1, IPC_CREAT | S_IRUSR | S_IWUSR);
    // 1. parameter - uniqe key
    // 2. parameter - the number of semaphores in semaphore group
    // 3. parameter - if it doesn't exist, then create with the given permissions
...
void sem_inic(int semaphore, int init)  // must inic before use
{
    ...
    // to set values of semaphore
    // 1. parameter - the semaphore id
    // 2. parameter - the ordinal number of semaphore, which I want to set new value
    // 3.parameter - SETVAL = means to set the value,
    // GETVAL = to read the value,
    // IPCRMD = to remove semaphore, e.g.
    // 4.parameter - the variable, which contains the semaphore value
...
void semoperation(int semaphore, int op)
{
    ...
    semstate[0].sem_num = 0;        // the ordinal number of semaphore in semaphore group, which I want to use
    semstate[0].sem_op = op;        // to modify the semaphore value, for closing
    semstate[0].sem_flg = 0;        // sem_flg = IPC_NOWAIT -> it will not wait, immediately comes back
