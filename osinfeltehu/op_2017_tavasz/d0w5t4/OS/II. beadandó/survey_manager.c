...
int num_choices (const struct Question* q)
{
  struct Question r = *q;

  int n = 0;
  while (r.opts[n][0] != '\0' && n < 4)
  {
    n++;
  }
  return n;
...
struct tm to_tm(time_t rawtime)
{
  struct tm timeinfo;
...
void print_question(const struct Question* pq)
{
  struct Question q = *pq;

  struct tm mod_date = to_tm(q.modified_date);
  char* date_str = asctime(&mod_date);
  del_newline(date_str);
  printf("%s (%s)\n %s\n %s\n %s\n %s\n",
          q.question, date_str,
          q.opts[0], q.opts[1], q.opts[2], q.opts[3]);
}

int load_nth_question(int n, struct Question* q)
{
    ...
    fseek(file, (n-1)*sizeof(struct Question), SEEK_SET);

    int res = fread(q, sizeof(struct Question), 1, file);
...
void on_child_ready(int signumber)
{
...
void send_survey()
{
  ...
  load_survey("Which survey to send?");

  int pipefd[2]; // unnamed pipe file descriptor array
  pid_t pid;
  struct Question q[3];  // Question array

  if (pipe(pipefd) == -1)
  {
      perror("Erro while opening the pipeline");
      exit(EXIT_FAILURE);
  }

  signal(SIGUSR1,on_child_ready);

  // we will block the SIGUSR1 signal
  sigset_t sigset;
  sigemptyset(&sigset);
  ...
  { // parent
    ...
    // now we will let the SIGUSR1 signal throw (and only SIGUSR1)
    sigfillset(&sigset);
	sigdelset(&sigset, SIGUSR1);
	sigsuspend(&sigset);  // now equivalent to pause()

    // the child is read to work
    ...
    for (int i = 0; i < 3; i++)
    {
      load_nth_question(i+1, q+i);
      write(pipefd[1], q+i, sizeof(struct Question));
    }
    sigsuspend(&sigset);
    read(pipefd[0], choices, sizeof(choices));

    for (int i = 0; i < 3; i++)
    {
      printf("%s\n", q[i].question);
      for (int k = 0; k < num_choices(q+i); k++)
      {
        printf("%d: %d\n", k+1, choices[i][k]);
      }
    }
    ...
    int status;
    wait(&status); // exit code of child
  }
  else
  { // child
    ...
    for (int i = 0; i < 3; i++)
    {
      read(pipefd[0], &q, sizeof(struct Question));
      int n = num_choices(&q);
    ...
    write(pipefd[1], choices, sizeof(choices));
    kill(getppid(),SIGUSR1);
