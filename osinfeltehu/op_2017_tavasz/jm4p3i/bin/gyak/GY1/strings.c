...
char* string_replace(char *source, char *search, char *replace) {
  ...
  if(loc) 
  {
      result = (char*)malloc( (strlen(source) + (strlen(search) - strlen(replace))) * sizeof(char) );
      result = strncpy(result, source, loc - source);
      result = strcpy(result + strlen(loc) - strlen(source), replace);
      result = strcpy(result + strlen(loc) - strlen(source) + strlen(replace), loc+strlen(loc));
  }
  return result;
}
...
int main()
{
  ...
  //char nev[100] = "alma"; //includes 0 char at the end
  ...
  str2 = string_replace(str2, "content", "aaaa");
