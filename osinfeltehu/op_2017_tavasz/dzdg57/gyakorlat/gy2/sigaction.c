...
int main(){
  ...
  if (child>0)
  {
    sigset_t sigset;
    sigfillset(&sigset);
    sigdelset(&sigset,SIGTERM);		// amik benne vannak, azok vannak blokkolva...töröltük SIGTERM-et... tehát csak a SIGTERM nincs már maszkolva
    sigsuspend(&sigset);			// csak a SIGTERM-re várakozik.... tehát felfüggesztjük a folyamatot, amíg ez meg nem érkezik
  ...
  else 
  {
    ...
    sleep(3);
    kill(getppid(),SIGUSR1);	// erre nem fog reagálni a szülő, hiszen ez maszkolva volt
    printf("Waits 3 seconds, then send a SIGTERM %i signal (it is waited for by suspend)\n",SIGTERM);		
    sleep(3);
    kill(getppid(),SIGTERM);	// erre már reagálni fog a szülő!
