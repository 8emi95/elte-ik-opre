...
struct messg { 
     ...
     int mtext[ 3 ]; //this is the message itself
}; 
...
{ 
	...
	// if  >0 (e.g. 5), then message type 5 will be read
     ...
     status = msgrcv(mqueue, &m,sizeof(m.mtext) , 1, 0 ); 
     
     if ( status < 0 ) 
          perror("msgsnd error"); 
     else
          printf( "The code of the message is : %ld, text is:  %i\n %i\n %i\n", m.mtype, m.mtext[0],m.mtext[1],m.mtext[2]); 
     return 0; 
} 
