...
glm::vec3	CMyApp::GetUV_henger(float u, float v)
{
	...
	return glm::vec3(r*cu, r*su, h*v);
}

glm::vec3	CMyApp::GetUV_torusz(float u, float v)
{
	// origó középpontú, egységsugarú gömb parametrikus alakja: http://hu.wikipedia.org/wiki/G%C3%B6mb#Egyenletek 
	// figyeljünk:	matematikában sokszor a Z tengely mutat felfelé, de nálunk az Y, tehát a legtöbb képlethez képest nálunk
	//				az Y és Z koordináták felcserélve szerepelnek
...
bool CMyApp::Init()
{
	...
	for (int i=0; i<=N; ++i)
		for (int j=0; j<=M; ++j)
		{
			...
			vert[i + j*(N+1)].p = GetUV_torusz(u, v);
...
void CMyApp::Update()
{
	...
	m_matView = glm::lookAt(glm::vec3( 10*cosf(t),  10,  10*sinf(t)),		// honnan nézzük a színteret
...
void CMyApp::Render()
{
	...
	*/
	...
	m_matWorld = glm::rotate<float>(0, glm::vec3(1, 0, 0));
