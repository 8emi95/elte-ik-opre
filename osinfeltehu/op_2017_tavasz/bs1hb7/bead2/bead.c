			...
			pid_t child1=fork();
				...
				if(child1> 0) {
  				...
  				else
  				{
					...
					printf("A kerdezeobitos megekerkezet %s\n",argv[1]);
							sleep(1);
					...
					for(j=0; j<3; j++){
						...
						printf("kerdes: %s\n",mkerd[j].kerd);
							...
							printf("1. valasz: %s",mkerd[j].va);
							printf("2. valasz: %s",mkerd[j].v2);
							printf("3. valasz: %s",mkerd[j].v3);
							printf("4. valasz: %s",mkerd[j].v4);
							sleep(1);
						kill(getppid(),SIGTERM);
