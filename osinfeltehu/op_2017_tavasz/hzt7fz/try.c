...
//Accepts SIGUSR1
		...
		if(child2 > 0){		
			...
			//Writing the process IDs to shared memory
			shared_memory[0] = child1;
			shared_memory[1] = child2;
			...
			printf("Parent sends SIGINT to child2\n");
			kill(child2, SIGINT);
		...
		//Accepts SIGINT
			...
			sem_wait(mutex);
			child2 = shared_memory[0];  //Acquiring the pid of the other child
			sem_post(mutex);
			
			//***************
			
			sigsuspend(&c2_sigset);
			printf("Child 2 wakes up\n");
			
			sigsuspend(&c2_sigset);
			printf("Child2 wakes up again");
	...
	//Accepts SIGUSR2
		...
		sem_wait(mutex);
		child2 = shared_memory[1];  //Acquiring the pid of the other child
		sem_post(mutex);
		
		//***************
		
		sleep(1);
		printf("Child sends SIGUSR1\n");
		kill(getppid(), SIGUSR1);
		
		printf("Sending SIGINT to child2.");
		kill(child2, SIGINT);
		
		printf("Child1 waits for SIGUSR2\n");
