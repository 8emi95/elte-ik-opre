...
struct mq_attr attr;
sigset_t set;
char *mqname = "/nagyablak"; // mqname must start with / !!!!!
...
if(pid == 0) // child
	...
	//mq_send(mqdes2, argv[1], MSGSIZE, 30);
        int pid = getpid();
        char* msg = malloc(1024);
        snprintf(msg, 1024, "%d", pid);
        strcat(msg, argv[1]);
    mq_send(mqdes2, msg, MSGSIZE, 30);
        sleep(3);
        mq_receive(mqdes1, rcv_buf, MSGSIZE, 0);
        printf("Child:\t\t\tQueue transition occured - received: %s\n",rcv_buf);
	...
	printf("Parent:\t\t\tQueue transition occured - received: %s\n",rcv_buf);
        mq_send(mqdes2, "Price", MSGSIZE, 30 );
	}
