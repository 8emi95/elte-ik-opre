//make a copy of file given in argv[0] to file given in argv[2] 
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc,char** argv){
 ...
 struct Person a[2] = { {"Beko Toni",33},{"Futy Imre", 24533445}};
 ...
 g=open(argv[2],O_WRONLY|O_CREAT|O_TRUNC,S_IRUSR|S_IWUSR);//S_IRUSR|S_IWUSR lehetne 600
 ...
 while (read(f,&c,sizeof(c))){ 
    //read gives back the number of bytes viszadja mit sikerül olvasnia bájt számot a 0 a hamis és minden más igaz
    //1. parameter the file descriptor honan olvasok
    //2. parameter the address of variable, we read into hova olvasok cimszerint
