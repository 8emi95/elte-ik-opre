...
bool CMyApp::Init()
{
	...
	{ 
		//          x,  y, z               nx,ny,nz			 s, t
		{glm::vec3(-2.5, 0, -2.5), glm::vec3( 0, 1, 0), glm::vec2(0, 0)},
		{glm::vec3(-2.5, 0,  2.5), glm::vec3( 0, 1, 0), glm::vec2(0, 1)},
		{glm::vec3(2.5, 0, -2.5), glm::vec3( 0, 1, 0), glm::vec2(1, 0)},
		{glm::vec3(2.5, 0,  2.5), glm::vec3( 0, 1, 0), glm::vec2(1, 1)},
	};
	...
	// textúra betöltése
	m_waterTextureID2 = TextureFromFile("saturn_preview.bmp");
	// textúra betöltése
	m_waterTextureID = TextureFromFile("ring_preview.tga");
	// textúra betöltése
	m_waterTextureID3 = TextureFromFile("titan.jpg");
	// textúra betöltése
	m_waterTextureID4 = TextureFromFile("rhea.jpg");
	// textúra betöltése
	m_waterTextureID5 = TextureFromFile("enceladus.jpg");
	// textúra betöltés
	// mesh betoltese
	m_mesh = ObjParser::parse("planet.obj");
	m_mesh->initBuffers();
...
void CMyApp::Update()
{
	...
	m_matView = glm::lookAt(glm::vec3(180, 180, 180),		// honnan nézzük a színteret
...
void CMyApp::DrawGround()
{
	...
	*/
	m_matWorld = glm::scale(glm::vec3(20, 20, 20))*glm::rotate<float>(M_PI*26.73 / 180.0f, glm::vec3(1, 0, 0));
...
void CMyApp::titan() {
	...
	m_matWorld = glm::rotate(-(SDL_GetTicks() / 6000.0f), glm::vec3(1, 1, 0))*glm::translate(glm::vec3(0, 100, 0))*glm::rotate<float>(M_PI*26.73 / 180.0f, glm::vec3(1, 0, 0))*glm::scale(glm::vec3(2, 2, 2));
	glm::mat4 mvp = m_matProj * m_matView * m_matWorld;
	// majd küldjük át a megfelelõ mátrixokat!
	glUniformMatrix4fv(m_loc_mvp,// erre a helyre töltsünk át adatot
		1,			// egy darab mátrixot
	...
	glActiveTexture(GL_TEXTURE0);
	// aktiváljuk a generált textúránkat 
	glBindTexture(GL_TEXTURE_2D, m_waterTextureID3);
	// textúra mintavételezõ és shader-beli sampler2D összerendelése
	glUniform1i(m_loc_texture,	// ezen azonosítójú sampler 2D
		0);				// olvassa az ezen indexû mintavételezõt
...
void CMyApp::Rhea() {
	...
	m_matWorld = glm::rotate(-(SDL_GetTicks() / 4000.0f), glm::vec3(1, 1, 0))*glm::translate(glm::vec3(0, 80, 0))*glm::rotate<float>(M_PI*26.73 / 180.0f, glm::vec3(1, 0, 0));
	glm::mat4 mvp = m_matProj * m_matView * m_matWorld;
	// majd küldjük át a megfelelõ mátrixokat!
	glUniformMatrix4fv(m_loc_mvp,// erre a helyre töltsünk át adatot
		1,			// egy darab mátrixot
	...
	glActiveTexture(GL_TEXTURE0);
	// aktiváljuk a generált textúránkat 
	glBindTexture(GL_TEXTURE_2D, m_waterTextureID4);
	// textúra mintavételezõ és shader-beli sampler2D összerendelése
	glUniform1i(m_loc_texture,	// ezen azonosítójú sampler 2D
		0);				// olvassa az ezen indexû mintavételezõt
...
void CMyApp::Enceladus() {
	// a mesh kirajzolasahoz hasznalt shader bekapcsolasa
	glUseProgram(m_programID);

	m_matWorld = glm::rotate(-(SDL_GetTicks() / 2500.0f), glm::vec3(1, 1, 0))*glm::translate(glm::vec3(0, 60, 0))*glm::rotate<float>(M_PI*26.73 / 180.0f, glm::vec3(1, 0, 0));
	glm::mat4 mvp = m_matProj * m_matView * m_matWorld;
	// majd küldjük át a megfelelõ mátrixokat!
	glUniformMatrix4fv(m_loc_mvp,// erre a helyre töltsünk át adatot
		1,			// egy darab mátrixot
	...
	glActiveTexture(GL_TEXTURE0);
	// aktiváljuk a generált textúránkat 
	glBindTexture(GL_TEXTURE_2D, m_waterTextureID5);
	// textúra mintavételezõ és shader-beli sampler2D összerendelése
	glUniform1i(m_loc_texture,	// ezen azonosítójú sampler 2D
		0);				// olvassa az ezen indexû mintavételezõt
