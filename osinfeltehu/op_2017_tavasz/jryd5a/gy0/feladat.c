...
int main(int argc,char** argv) {
  ...
  strcpy( structure.string, "some characters for testing" );
  structure.number = 42;

  FILE *write_ptr;

  write_ptr = fopen( "testfile", "wb" );

  fwrite( structure.string, sizeof(structure.string), 1, write_ptr );

//  int filedesc = open( "./testfile", O_WRONLY | O_TRUNC | O_CREAT, 0600 );

//  write( filedesc, &structure.string, sizeof( structure.string ) );
//  write( filedesc, &structure.number, sizeof( structure.number ) );
