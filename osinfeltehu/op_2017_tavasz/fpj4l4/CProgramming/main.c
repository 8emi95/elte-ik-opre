...
void listQuestions(){
    ...
    if(fp != NULL){
        while (fread(qb,sizeof(QB),1, fp)) {
            printf("questiontitle:%s questionnumber:%s questiondate:%s questiona1:%s questiona2:%s questiona3:%s questiona4:%s",
                   qb->question,qb->qNumber,qb->qctime,qb->answer1,qb->answer2,qb->answer3,qb->answer4);
        }
        fclose(fp);
    }
