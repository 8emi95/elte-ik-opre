...
struct data proc_line(char * line){
   ...
   struct data rekord = {arr[0],arr[1],arr[2],arr[3],arr[4],arr[5],arr[6]};
   return rekord;
}
...
void del_que(char * fname, int del_num){
    ...
    while (!feof(f)){
        ...
        if (atoi(tmp.id) != del_num){
            fprintf(f2, "%d;%s;%s;%s;%s;%s;%s",atoi(tmp.id),tmp.date,tmp.kerdes,tmp.valasz1,tmp.valasz2,tmp.valasz3,tmp.valasz4);
        }
    }
    fclose(f);
    fclose(f2);
...
void list_ques(char* fname){
    ...
    while (!feof(f)){
        ...
        tmp = proc_line(line);   
        i++;
        printf("%d.: %s %s/%s/%s/%s\n",atoi(tmp.id),tmp.kerdes,tmp.valasz1,tmp.valasz2,tmp.valasz3,tmp.valasz4);
    }
    fclose(f);
}
    ...
    while (ch_num != 0){
        ...
        while (!feof(f)){
            ...
            if (atoi(tmp.id) == ch_num){
                fprintf(f2, "Kerdes: %s \n Valaszok: %s/%s/%s/%s",tmp.kerdes,tmp.valasz1,tmp.valasz2,tmp.valasz3,tmp.valasz4);
            }        
        }
        fclose(f);
    }
