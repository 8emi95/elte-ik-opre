...
int main (int argc,char* argv[]) {
 ...
 s = shmat(oszt_mem_id,NULL,0);
    ...
    if(pid>0){    
       ...
       int buffer[2] = {1996, 0124};          
       // beirunk a memoriaba 
       strcpy(s, (const char*)&buffer[0]);
       strcpy(s, (const char*)&buffer[1]);
...
//	s[0]='B';  //ez segmentation fault hibat ad
	...
	sleep(1);	              
		int buffer[2];
		strcpy((char*)&buffer[0], s);
		strcpy((char*)&buffer[1], s);
        printf("A gyerek ezt olvasta az osztott memoriabol: %i, %i\n",buffer[0], buffer[1]);
	// gyerek is elengedi
        shmdt(s);
	}
