...
{
    int eventfile;
    eventfile = open("eventfile", O_WRONLY|O_CREAT|O_APPEND, 0700);
    write(eventfile, '\0', 0);
    close(eventfile);  
}
...
void deleteMember()
{
    ...
    int g;
    g = open("mas", O_RDWR| O_CREAT | O_APPEND, 0700);
...
void deleteEvent(){
    ...
    int g;
    g = open("eventtmp", O_RDWR| O_CREAT | O_APPEND, 0700);
    if(g < 0) { perror("Error"); exit(1); }
	...
	int f;
    f = open("memberstmp", O_RDWR| O_CREAT | O_APPEND, 0700);
    if(f < 0) { perror("Error"); exit(1); }
