...
// posixos szemafor, egyszerubb
// itt 1 szemafor van, nem csalad, gyakran több kell
...
int main (int argc,char* argv[]) {
	...
	if ( child == 0 ) //child
		...
		printf("Gyerek: Indul a posix szemafor down!\n");
			//var hogy leengedhesse a szemafort, de elotte a szulore var hogy az felengedje
			//	majd kezarja maga szamara, kiolvas, folengedi
       		sem_wait(semid);	// semaphore down
       		printf("Gyerek, down rendben, eredmeny: %s",s);  
       		sem_post(semid);      
       		/*  kritikus szakasz vége  */  	  
			//lekapcsolodnak az osztott memoriarol
       		shmdt(s);
    		}
