...
int szemafor_letrehozas(const char* pathname,int szemafor_ertek){
	...
	//mindig egy szemaforcsaladot hozunk letre, itt 1 et, ez az 1es prameter:
    if((semid=semget(kulcs,1,IPC_CREAT|S_IRUSR|S_IWUSR ))<0)
	perror("semget");
    // semget 2. parameter is the number of semaphores   
	//semctl inicializalas, ezelott nem illik hazsnalni:
	//(csalad, szemafor sorszama,setval: ugyanezzel a fv el lehet torolni is...)
