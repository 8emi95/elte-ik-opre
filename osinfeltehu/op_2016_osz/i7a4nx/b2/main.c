...
//figyeli a @ - . karakterek poziciojat
...
//csak addig olvassa a visitorsData fájl, amíg azt meg nem találja
 ...
 // vissszaadja az emailcímet hordozó karakterláncot     
  ...
  // vissszaadja a rendezvény nevét hordozó karakterláncot     
   char* getEventName(int eventID, char* eventName){
 ...
 // vissszaadja a vendég nevét hordozó karakterláncot     
   void getGuestName(int guestID, char* guestName){
...
void list(){
    ...
    printf("\n rend.ID \t\t    RendezvenyNev \t\t helyszin \t\t kezdes");
    printf("\n ------- \t\t    ------------- \t\t -------- \t\t ------  \n");   
    while ( fread( &event, sizeof(event), 1, ptr_eventsFile) ) {
        printf("\n %5d %25s  \t%20s\t\t%30s", event.eventID, event.eventName, event.location, ctimeToStr(event.kezdes));        
    }
    printf("\n\n");   
...
int listGuests(int eventID, int filehossza ) {
    ...
    for(i=0; i<darab; ++i){
        ...
        getGuestName(egyezik[i].guestID, guestName);
...
void createEvent(){
    ...
    printf("\nAz uj rendezveny helyszine: ");
    fgets(location, 50, stdin);
    removeNewLine(location);
    
    printf("\nAz uj rendezveny kezdesi ideje: \n");
    ...
    //printf("%d-%d-%d %d:%d", year, mon, day, hour, min);
    //tm szabvanykonyvtari struct segitsegevel ctime tipusu valtozot keszitunk
    ...
    startTime = mktime( &str_time );
                
    //printf("\n A time valtozoban: ");
    //rintf(ctime(&startTime));
    
    year=mon=day=hour=min=0;
    memset(temp,0,10);
    
    //rogzitjuk az esemenyek fajljaban   
    ptr_eventsFile = fopen( "eventsData" , "a+b" );
    if (ptr_eventsFile==NULL){
        perror("! eventsData nyitási hiba !\n"); 
        exit(1);
    ...
    event.location[strlen(location)]='\0';
    
    event.kezdes = startTime;
...
//egyeni osszehasonlito fv a levezenyel() qsort() -jahoz, amivel 
//a rendezvenyek structjainak tombjet rendezzuk a rendezveny 
//kezdesenek ideje szerinti novekvo sorrendbe
int cmpbyTimeFunc( const void* a, const void* b ){
    
    EVENTS* eventA = (EVENTS*)a; 
    EVENTS* eventB = (EVENTS*)b;
    ...
    if ( difftime( timeA, timeB ) > 0 ) { return 1; }
    if ( difftime( timeA, timeB ) > 0 ) { return 0; }
    if ( difftime( timeA, timeB ) < 0 ) { return -1; }    
}

//szignalkezello fuggveny
//nehol varakoztatom a folyamatokat, mig megerkezik
//a szukseges adat, az ezutan valo ebresztesre
...
//masodik beadando fo fuggvenye:
//megnezi milyen rendezvenyregisztraciok vannak, 
//ezt osszefuttatja az esemenyek es a vendegek adatfajljaval
//qsort() + egyedi osszehasonlito fv el idorendi sorrendbe rendezi az
//esemenyek evarr[] tombjet,
//ezen vegigiteralva gyerekfolyamat indul minden rendezveny sorrendi lebonyolitasahoz
//hasznal pipeot, signal SIGUSR2 + handlert
//osztott memoriat karakterlancok tombjenek megosztasara a szulo es gyerek kozott
    //ebbe kerulnek a rendezvenyen meg nem jelentek
//a rendezveny sikeressegenek random szamat 1elemu uzenetsorkent adja vissza a szulonek
int levezenyel(int eventsFileLen, int regFileLen) {
    int err = -1;
    int i, eventIndex;
    int darab = 0;
    int uzenetsor, uzenet_status; 
    EVENTS evarr[eventsFileLen];
    EVENTS event;
    REGISTRATIONS reg;
    REGISTRATIONS egyezik[regFileLen];
    ...
    for(i = 0; i < eventsFileLen-1; i++){
        fread(&event, sizeof(event), 1, ptr_eventsFile);
        evarr[i] = event;     
    }
    fclose(ptr_eventsFile);
    
    //printf("EVFL %d, RFL %d, SGN %d ", eventsFileLen, regFileLen, sizeof(guestName));
    
    //qsort() al idorendbe rakjuk a rendezvenzeket
    /* //qsort teszt  
    for(i = 0; i <eventsFileLen-1;i++){
        printf (" %s ", ctimeToStr(evarr[i].kezdes) );
    }
    printf ("\n " );
    */    
    qsort( evarr, eventsFileLen-1, sizeof(evarr[0]), cmpbyTimeFunc );
    /*
    for(i = 0; i <eventsFileLen-1;i++){
        printf (" %s ", ctimeToStr(evarr[i].kezdes) );
    }
    */
    ...
    //ez a ciklus vegigiteral a qsorttal rendezett tombon
    //idorendben feldolgozza a rendezvenyeket
    //elinditja a levezeto gyerekfolyamatokat
    for( eventIndex = 0; eventIndex < eventsFileLen-1; eventIndex++ )
    {        

        int status; 
        int fd = mkfifo("helyszint", S_IRUSR | S_IWUSR );
        int fd2 = mkfifo("fel", S_IRUSR | S_IWUSR );

        //uzenetsorhoz valtozok
        key_t kulcs;
        kulcs = ftok("mostebbolgeneraljuk",1); 
        uzenetsor = msgget( kulcs, 0600 | IPC_CREAT );     
        if ( uzenetsor < 0 ) { perror("msgget");} 

        //osztott memoriaval kapcsolatos valtozok
        char* sharedP;
        key_t memkulcs;
        int shmid;
        memkulcs=ftok("memkulcshoz",1);
    
        shmid=shmget( memkulcs, regFileLen * sizeof(guestName) , IPC_CREAT|S_IRUSR|S_IWUSR );
        sharedP = (char *) shmat(shmid, NULL, 0);
       // memset( sharedP, 0, regFileLen * 40 );

        //pipe polling ellenorzeshez
        struct pollfd poll_fds[5];
        
            int eventID = evarr[ eventIndex ].eventID;
            int talalat = 0;
            ...
            //ha senki nem jelentkezett vegul az adott rendezvenyre
            //jelezzuk, hogy az elmarad...
            //ez esetben nem indul gyerekfolyamat
            //hanem continue utasitas, jon a kovetkezo iteracio
            while ( fread(&reg, sizeof(reg), 1, ptr_regFile ) ) {
                if ( reg.eventID == eventID ) {              
                    talalat = 1;
                    break;              
            ...
            fclose(ptr_regFile);             
            //uj iteracio jon de itt is elengedjuk az eroforrasokat, nincs regisztralt vendeg eseten
            if (talalat == 0 ) { 
                printf("\n A %s -ben tartando, %s kezdetu %s rendezveny elmarad, nem jelentkezett fel ra senki !",evarr[ eventIndex ].location, ctimeToStr(evarr[ eventIndex ].kezdes), evarr[ eventIndex ].eventName );                
                printf("\n\n ------------------------------------------------------------------------------------------------------------\n" );
                
                 uzenet_status = msgctl( uzenetsor, IPC_RMID, NULL ); 
              if ( uzenet_status < 0 ) perror("msgctl"); 
       ...
       // a szulo folyamat
        if (child > 0){
            ...
            //a szulo atkuldi a gyereknek a kovetkezo rendezveny helyszinet
            fd = open("helyszint", O_WRONLY);
            write(fd, evarr[ eventIndex ].location, sizeof(evarr[ eventIndex ].location) );
            close(fd);
            unlink("helyszint");

            //megvarjuk, hogy a gyerek jelezze hogy megkapta
            //es kesz lebonyolitani a rendezvenzt
            //SIGUSR2 + jelzeskezelo szakitja meg a varakozast
            done_waiting = 0;
            while( !done_waiting ) { sleep(1); }

            //vegigolvassuk a regisztraciok fajljat es a kovetkezo esemeny resztvevoit atkuldjuk
            //a gyereknek, aki pollingol egy pipeot es ugy fogadja az adatokat
            printf("\n Szulo: Kuldom a rendezvenyre regisztraltak listajat!");         
            fflush(stdout); 

            eventID = evarr[ eventIndex ].eventID;
            ...
            fd2 = open("fel", O_WRONLY);
            darab = 0;
            while ( fread(&reg, sizeof(reg), 1, ptr_regFile ) ) {
                if ( reg.eventID == eventID ) {              
                    egyezik[darab] = reg;
                    getGuestName(egyezik[darab].guestID, guestName);
                    write(fd2, &guestName, strlen(guestName));
            ...
            //szulo uzenetben megkap egy veletlenszamot a rendezveny sikeresseget
            struct uzenet uz; 
            int status; 
            status = msgrcv(uzenetsor, &uz, sizeof( uz.rendezveny_sikeresseg ), 5, 0 ); 
            if ( status < 0 ) {
                perror("msgsnd");
            }
            else
             printf( "\n\n Szulo: jelentes erkezett, a rendezveny sikeressege : %d szazalek.\n", uz.rendezveny_sikeresseg ); 

            //szulo listazza a rendezvenzt kihagyokat, osztott memoriabol
            int nemJelentekMeg = atoi(sharedP);

            if (nemJelentekMeg > 0) {
                printf("\n Szulo: A rendezvenyen %i szemely nem jelent meg, a listajuk : \n", nemJelentekMeg);
                for(i = 1; i<=nemJelentekMeg; i++ )
                    printf(" \n\t %d: %s",i, sharedP + (i*sizeof(guestName)));
            } else {
                printf("\n Szulo: A rendezvenyen minden feliratkozo megjelent. \n");
            }
            fflush(stdout); 

            //gyerekre varas, eroforrasok elengedese
        ...
        // a gyerekfolyamat
            ...
            char helyszin[50];
            char guestName[40];        
            RESZVETEL jelenleti_iv[regFileLen];   
            ...
            //pipeon megkapjuk a szulotol az aktualis helyszint
            fd = open("helyszint", O_RDONLY);
            read(fd, helyszin, sizeof(helyszin) );
            close(fd);

            printf("\n Gyerek: Itt vagyok %s -ben, a rendezveny helyszinen, várom a résztvevőket !", helyszin );
            printf("\n         rendezveny neve: %s,         kezdete: %s", evarr[ eventIndex ].eventName, ctimeToStr(evarr[ eventIndex ].kezdes) );
            fflush(stdout); 

            //jelzunk a szulonek, hogy mehet a rendezveny
            kill( getppid(), SIGUSR2 );

            //pipeon megkapjuk a regisztraltak listajat
            fd2 = open("fel", O_RDONLY);

            poll_fds[0].fd = fd2;
            poll_fds[0].events = POLLIN;                       

               int result = poll(poll_fds, 1 , -1 );
               while( result ){

                   if(poll(poll_fds, 1 , -1 )){

                        if (poll_fds[0].revents & POLLIN){
                            memset(guestName,0,sizeof(guestName));
                            read(fd2, &guestName, sizeof(guestName) );

                            strcpy( jelenleti_iv[counter].guestName, guestName );
                            counter++;
                ...
                //kepernyore irjuk a regisztraltak listajat
                printf("\n\n Gyerek: a feliratkozottak: ");
                printf("\n (mellette teszteleshez a szampar: 0-100 veletlenszam es ennek megfeleloen 0/1 (nem vett reszt/reszt vett) \n ");

                srand(time(NULL));
                int j=0;
                for(i=0; i < counter; i++){
                    //random szamok 0-100 kozott
                    //0-90      tartomanyban jelen van a resztvevo
                    //91-100    kihagyja a rendezvenyt
                    randNr = rand() % 101;

                    if( randNr < 91 ) { jelenleti_iv[i].jelen = 1; }
                    else {
                        j++;
                        jelenleti_iv[i].jelen = 0; 

                        //osztott memoriaba kerul a rendezvenyt kihagyok neve
                        memcpy(sharedP + (j*sizeof(guestName)) , &jelenleti_iv[i].guestName, sizeof(guestName));  
                    }

                    printf("\n\t %s  %d  %d", jelenleti_iv[i].guestName, randNr, jelenleti_iv[i].jelen );
                }
                //shared[0] ba irjuk a meg nem jelentek szamat, hogy a szuloben 
                //egyszeruen vegigiteralhassunk rajtuk
                sprintf( sharedP, "%d", j );

                fflush(stdout);

                //a rendezveny sikeressege %ban, üzenetben küldjük vissza:
                randNr = rand() % 101;
                const struct uzenet uz = { 5, randNr }; 
                int uz_status; 

                uz_status = msgsnd( uzenetsor, &uz, sizeof( uz.rendezveny_sikeresseg ) , 0 ); 
        ...
        else { perror("Fork hiba!"); }
    
    }   //for eventIndexeken vege
...
//menü megjelenítése és elágaztatás a választott opció szerint
	...
	printf(" 8 - Teljes adathalmaz megjelenitese \n\n");
        ...
        printf(" 9 - Rendezvenyek levezenylese \n\n");
	...
	switch(choice){
	//esemeny id szerinti listazasa 1 esemenynek
	case 1:
	...
	case 2:
            createGuest();
            break;
	//letezo vendeggel rendezvenyre regisztralunk
        //GUEST struct email mezoje 50 hosszu
	case 3:
	...
	case 5:
        ...
        case 6:
            createEvent();
            break;
        //rendezveny torlese... a torlesek tovabbgyuruznek, mivel tobb adatfajlt hasznalok
        case 7:
            regFileLen = fajlhossz("regData");
            ...
            break;        
        //minden adatfajl formazott kiirasa teszt cellal
	case 8:
            list();
            break;
        //masodik beadando: rendezvenyek sorrendi lebonyolitasa    
        case 9:
            eventsFileLen = fajlhossz("eventsData");
            regFileLen = fajlhossz("regData");
            levezenyel( eventsFileLen, regFileLen );
            break;
        //kilepes    
        case 10:
            newrun = 0;
