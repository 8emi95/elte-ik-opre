		...
		if(child > 0){
			...
			waitpid(child, &status,0);
				...
				if(i == argc-1) {
					int bIter;
					char rcv_bu[50];
					for(bIter=0; bIter<bukottDB; bIter++){
						pid_t potvizs;
						potvizs = fork();
						...
						if(potvizs>0){
							feladatSorszama = (rand()%4)+1;
							fd=open("adatcso", O_WRONLY);
								write(fd, bukottak[ bIter ], sizeof(bukottak[bIter]));
							close(fd);
							waitpid(potvizs, &status, 0);
						}else if (potvizs==0) {
							fd=open("adatcso", O_RDONLY);
								//read(fd, rcv_bu, sizeof(rcv_bu));
								printf("\n ajaj");
							close(fd);
							exit(0);
						}else{ printf("Fork hiba");}
					}
				}
