...
int main() 
{
	...
	for(;;)
	{
		...
		write(1,"7. Rendezveny lebonyolitasa\n",28);
		write(1,"0. Kilepes\n",11);
...
void run_event(struct visitor visitor_arr[], int events_arr[], int *visitorcount , int *eventcount)
{
	...
	for(;;)
	{
		//választható rendezvények listázása
		printf("\nValaszthato rendezvenyek: \n");
		int i;
		for(i = 0; i < *eventcount; i++)
		{
			printf("%d.%d\n", i+1, events_arr[i]);
		...
		if (pid == 0) //child / partner
		{
			...
			printf("Partner: Megkaptam a rendezveny azonositojat: %d\n", recieved_id);
			
			printf("Partner: Utazas a helyszinre...\n");
			sleep(3);
			
			printf("Partner: Megerkeztem, varom a resztvevoket\n");
			...
			for (i = 0; i < attendant_count; i++)
			{
				...
				printf("%s ", attendants[i].email);
				printf("%d ", attendants[i].id);
				printf("%s", ctime(&attendants[i].time));
			}
			
			printf("Partner: Rendezveny lebonyolitasa...\n");
			sleep(3);
			
			printf("Partner: A rendezveny sikere: %d/10\n",rand() % 10);
			
			for (i = 0; i < attendant_count; i++)
			{
				if (rand()%10==1) printf("%s nem jelent meg", attendants[i].name);
			}
		}
		else //parent / központ
		{
			close(pipefd[0]);
			printf("\nKozpont: Azonosito kuldese: %d\n",events_arr[id-1]);
			write(pipefd[1], &events_arr[id-1], sizeof(int));
			
			pause();
			
			printf("Kozpont: Resztvevok kuldese\n");
