...
void modifyPerson(char id[50]){
	...
	people[i].time = t_now;
        if (strcmp(people[i].name, mod_name) == 0){
            ...
            scanf("%s", &email);
	    t_n = time(0);
            fprintf(ftemp, "%s %s %s %d\n", name, email, people[i].eventID, (int)t_n);
        } else{
            fprintf(ftemp, "%s %s %s %s\n", people[i].name, people[i].email, people[i].eventID, people[i].time);
        }
        ++i;
    ...
    printf("Person successfully modified!\n");
}
