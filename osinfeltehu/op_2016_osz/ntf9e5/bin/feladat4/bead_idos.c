...
void modifyPerson(char id[50]){
    ...
    if (fp == 0){
        ...
        if (strcmp(people[i].name, mod_name) == 0){
            ...
            scanf("%s", &email);
			t_now = time(0);
            fprintf(ftemp, "%s %s %s %s\n", name, email, people[i].eventID, ctime(&t_now));
        } else{
            fprintf(ftemp, "%s %s %s %s\n", people[i].name, people[i].email, people[i].eventID, ctime(&people[i].time));
        }
        ++i;
    ...
    if (ftemp == 0){
        ...
        fprintf(fp, "%s %s %s %s\n", name, email, eventID, ctime(&t_now));
        }
    }
