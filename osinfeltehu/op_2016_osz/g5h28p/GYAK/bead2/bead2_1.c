...
int main() {
    ...
    while(strlen(fgets (buffer, sizeof (buffer), fileptr1)) > 3)
     {
     	char *p = strtok (buffer, " ");
                ...
                strcpy(rendezveny.azon, array[0]);
                strcpy(rendezveny.nev, array[1]);
                rendezveny.jelentkezokszama = atoi(array[2]);
	    ...
	    if (pid == 0)
	    {
	        ...
	        else
	        {
	            ...
	            printf("Rendezveny(%i): %s %s %d\n",pid, rendezveny_child.azon, rendezveny_child.nev, rendezveny_child.jelentkezokszama);
		    ...
		    if(write(fd[P2_WRITE], &random_number, sizeof(random_number)) < 0)
            {
                perror("Child: Failed to write response value");
                exit(EXIT_FAILURE);
            }
	    ...
	    {
	        // report what we received
	        printf("Megbizott visszajelzese: %s\n", buff);
	        fileptr2 = fopen("latogato.db", "r");
	        struct Latogato latogato;
	        while(fscanf(fileptr2,"%s %s %s %s %d", latogato.rendezvenyAzon, latogato.vezeteknev, latogato.keresztnev, latogato.email, &latogato.datum)!= EOF)
	        {
	    ...
	    int sikeresseg;
	    read(fd[P1_READ], &sikeresseg, sizeof(sikeresseg));
	    printf("Rendezveny sikeressege %i%%\n", sikeresseg);
