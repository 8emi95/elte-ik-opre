...
int main()
{
  ...
  //honnatól akarunk zárolni: (itt a fájl elejétől)
  ...
  //hány bájttól eltolva akarjuk a zárolást (itt a legelejétől)
  lock_data.l_start=0; // relative position of locking to l_whence
  //miylen hosszan (itt a teljes fájlt akarom zárolni)
  ...
  if (child>0)
    { //Parent process
      ...
      { 
	   ...
	   //melik folyamat fog zárolni
          lock_data.l_pid=getpid();    //which process locks
          //miylen módon akarom zárolni 
		  lock_data.l_type=F_WRLCK;    //for what it is locked F_WRLCK-write,FRDLCK-read,F_UNLCK - unlock
          //itt zűroljuk ténylegesen a fájlt (fcntl függvény hívása)
		  rc=fcntl(f,F_SETLKW,&lock_data); //F_setlkw - locks file for writing and it is waiting, F_SETLK - it locks if it can, but not wait (melyik fájl, mit akarok, struktúra)
          struct flock l2;
          if (rc!=(-1))
          {        
           for (i=0;i<length1;i++)
