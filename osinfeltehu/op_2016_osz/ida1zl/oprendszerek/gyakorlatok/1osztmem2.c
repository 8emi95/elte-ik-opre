...
int main (int argc,char* argv[]) {
    ...
    kulcs=ftok(argv[0],1);	//16 szám lehet egy azonosítóhoz
    // osztott memoria letrehozasa, irasra olvasasra, 500 bajt mrettel
    oszt_mem_id=shmget(kulcs,500,IPC_CREAT|S_IRUSR|S_IWUSR); //2. mekkor osztott memóriára van szükség, 3. jogosultságok
 ...
 s = shmat(oszt_mem_id,NULL,0);	//felkapcsolódás az osztott memóriára, címet ad vissza ahol elérjük az osztott memóriát
    //fork előtt, tehát ugynaazt látja a szülő és a gyerek is
    pid = fork();
    if(pid>0){    
