...
int main(){
  ...
  //parameters, how: SIG_BLOCK, SIG_UNBLOCK, SIG_SETMASK -   ; mit akarunk: blokkolni, feloldani
  //2. parameter changes the signalset to this if it is not NULL, mit
  ...
  if (child>0)
  {
	  //sigterm-nél továbbra se fut le csak a pause
  ...
  else 
  {
    ...
    sleep(3);
	//előbb a sigusr1 megy át, az feloldja a blokkolás így utólag érkezik meg a sigterm
    kill(getppid(),SIGTERM);
    printf("Waits 3 seconds, then send a SIGUSR1 %i signal (it is not blocked)\n", SIGUSR1);
    sleep(3);
    kill(getppid(),SIGUSR1); //ez nincs blokkolva, megy a szülőnek
