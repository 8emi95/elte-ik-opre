...
//sa_mask: megmondja milyen jelzéseket blokkoljon  
  //now only the arriving signal, SIGTERM will be blocked
  sigact.sa_flags=0; //nothing special behaviour
  sigaction(SIGTERM,&sigact,NULL);		//többet tud a signal-nal
  ...
  if (child>0)
  {
    sigset_t sigset;
    sigfillset(&sigset);
    sigdelset(&sigset,SIGTERM);//kivesszük a halmazból a sigetrm jelzést, mert ezt nem akarjuk blokkolni
    sigsuspend(&sigset); //pause helyett, több: be lehet állítani a blokkolást - mire várjon, mire ne
  ...
  else 
  {
    ...
    sleep(3);
    kill(getppid(),SIGUSR1);//ez blokkolva van, előbb a sigterm érkezik majd, az feloldja a blokkolást és megérkezik a sigusr1
