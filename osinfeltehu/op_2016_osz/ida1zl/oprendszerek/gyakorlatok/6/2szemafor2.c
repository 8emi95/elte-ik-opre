...
int szemafor_letrehozas(const char* pathname,int szemafor_ertek){
    ...
    kulcs=ftok(pathname,1);    //kell
    if((semid=semget(kulcs,1,IPC_CREAT|S_IRUSR|S_IWUSR ))<0)	//hány tagból áll a család
	perror("semget");
...
void szemafor_muvelet(int semid, int op){
    ...
    //melyik szemaforral mit akarunk kezdeni
    muvelet.sem_num = 0;	//szemafor sorszáma
    muvelet.sem_op  = op; // op=1 up, op=-1 down; mit csináljunk a szemaforral
    muvelet.sem_flg = 0;
