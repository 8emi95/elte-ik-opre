...
int main(int argc, char* argv[])
{
    int pid,fd;
    printf("Fifo start!\n");
    int fid=mkfifo("fifo.ftc", S_IRUSR|S_IWUSR ); // creating named pipe file, kellenek a jogosultságok
	...
	//read(fd,s,sizeof(s)); nem olvasunk ki a csőből, hiba!
	printf("Ezt olvastam a csobol: %s \n",s);
	close(fd);
	// remove fifo.ftc
	unlink("fifo.ftc");	//törli a csővezetéket, nem tudná újra létrehozni, KELL!!!
        }
	else // child
	{
