...
#include "fork.h" /* all things related to handling events with child processes */
    ...
    do
    {
       ...
       { 
		   ...
		   printf("Error opening data file for writing!\n");
               ...
               break;
       case 'A':
	   for(unsigned eventInd = 0; eventInd != event_count; ++eventInd)
	   {
	       handleEventWithChild(entries,entry_count,&events[eventInd]);
	   }
	   break;
           case '\n':
               goto scan;
