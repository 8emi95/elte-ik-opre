...
int main(int argc, char *argv[])
{
  ...
  if (pid == 0)
  {
    ...
    kill(getppid(), SIGTERM);
	 printf("Gyerek SIGTERM signalt kuldott a szulonek\n");
    sleep(3);
    kill(getppid(), SIGUSR1);
	printf("Gyerek SIGUSR1 signalt kuldott a szulonek\n");
  ...
  else
  {
	...
	printf("Szulo a csobe irta: %s\n", convertSignalToStr(signalFlag));
    pause();
    write(pipefd[1], convertSignalToStr(signalFlag), 20);
	printf("Szulo a csobe irta: %s\n", convertSignalToStr(signalFlag));
    close(pipefd[1]); // Closing write descriptor
    printf("Szulo minden adatot beirt a csobe!\n");
