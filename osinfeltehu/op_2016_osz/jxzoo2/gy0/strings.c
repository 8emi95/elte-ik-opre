...
int length(char * str) //kezdetben ez mutat a char tomb 1. elemere
{
 int l=0;
...
//char str1[80]; //it would be an 80 characters long empty "string", - it is empty 
  ...
  char* str2; //it is a pointer variable (we have to allocate memory) meg nem mutat sehova
  ...
  str2=(char*)malloc(80*sizeof(char)); //allocate new memory malloc fv hivasaval. param: hany byte-ot akarok lefoglalni.
				       //tipuskenyszerites: (char*)malloc...
  ...
  strcpy(str2,"New content by using strcpy");   //1.param: hova, 2.param: mit masolunk
