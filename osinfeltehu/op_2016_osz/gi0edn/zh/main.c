...
void signal_handler(int signum)
{
	...
	{
		case SIGUSR1:
			printf("Uzemelteto kapta: sikeresen beert a szerelveny!\n");
			break;
		case SIGUSR2:
			read(pipefd2[0], &buf, sizeof(buf));
			printf("Uzemelteto kapta: %s\n ",buf);
			break;	
	}
...
int main(int argc, char **argv)
{
	...
	if (childpid == 0) // child
	{
		...
		printf("Ez a(z) %d. szerelveny, meg %d db lesz ezutan, %s teljesitette az uzemet.\n",
	...
	else // parent
	{
		...
		write(pipefd[1], &l_info, sizeof(line_info));
