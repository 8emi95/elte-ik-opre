...
int main(){
  ...
  //if real time signals are sent, the for cycle of sending signals will stop, because sleep is not thread safe!!
  //so only 1 signal will be sent - with sa_flags=0!!
  //using sa_flags=SA_RESTART, the for cycle of sending signals will continue 
 ...
 //************************************************  
  ...
  else 
  {
    ...
    //They are not blocked - they loose!
...
//       kill(getppid(),35);
//       sleep(1);  
     //if sleep is working, then sigusr1 has done before the next call performs
    }
