...
int main(){
 ...
 struct stat st;
 d=opendir("."); //parameter - the directory name (with path), which we want to open
 if (d<0){perror("Some problem in opening");exit(1);}
 while (dp=readdir(d)){ 
  
   stat(dp->d_name,&st);  //fstat(file descriptor, stat structure)
   if (S_ISDIR(st.st_mode)) {
     //S_ISDIR returns true, if it is a directory
     printf("d:");
   }
   else {
 ...
 closedir(d); //close directory, use after opendir
 return 0;
}
