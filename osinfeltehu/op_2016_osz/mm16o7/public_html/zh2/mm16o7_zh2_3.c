...
{ 
  ...
  int kevinIrPipe[2];
  int betoroIrPipe[2];
  if (pipe(kevinIrPipe) == -1) 
  {
	perror("Hiba a pipe nyitaskor!");
	exit(EXIT_FAILURE);
  }
  if (pipe(betoroIrPipe) == -1) 
  {
	perror("Hiba a pipe nyitaskor!");
	exit(EXIT_FAILURE);
  }
  ...
  if ( kevin > 0 ) //PARENT
  {
	...
	else if (betoro == 0)	//BETORO
	{
		...
		char buff[100];
		char buff2[100];
		read(kevinIrPipe[0], buff, sizeof(buff));
		printf("%s\n", buff);
		...
		if(hatasos == 0)
		{
			sprintf(buff2,"%s","na megallj csak");
		}
		else
		{
			sprintf(buff2,"%s","nem uszod meg szarazon");
		}
		write(betoroIrPipe[1], buff2, sizeof(buff2));
		
		close(kevinIrPipe[0]);
		close(betoroIrPipe[1]);
		exit(EXIT_SUCCESS);
  ...
  { 
	...
	char buff[100];
	sprintf(buff,"%s","festek");
	write(kevinIrPipe[1], buff, sizeof(buff) );
	
	char buff2[100];
	read(betoroIrPipe[0], buff2, sizeof(buff2) );
