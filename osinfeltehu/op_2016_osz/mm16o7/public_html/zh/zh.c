...
void ParentProc(int child1Pid, int* child1Pipe, int* child1PipeBack, char* player)
{
	//parent
...
void ChildProc(int* child1Pipe, int* child1PipeBack)
{
	...
	strcpy(success ,rand() % 2 == 1 ? "successfull" : "unsuccessfull");
...
int main(int argc, char* argv[])
{
  ...
  sigaction(SIGUSR1,&sigact,NULL);
	...
	{
		ParentProc(child1, child1Pipe, child1PipeBack, argv[1]);
	}
	else
	{
		//child1
