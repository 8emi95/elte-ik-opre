...
int main (int argc,char* argv[]) {
    ...
    oszt_mem_id=shmget(kulcs, 500, IPC_CREAT|S_IRUSR|S_IWUSR); // 500 bite memoriat foglal
 ...
 s = shmat(oszt_mem_id,NULL,0); // ezzel csatlakozunk az osztott memoriahoz, az s cimen keresztul mindenki latja a memoriateruletet
    // miutan felcsatlakoztunk, semmi sem kulonbozteti meg s-et egy sima valtozotol
    pid = fork();
    if(pid > 0){
	...
	if (0 == pid) {
    	sleep(1); // Ez arra var, hogy beleirjak az osztott memoriaba amit ki kell olvasni, helyette szemafort erdemes hasznalni
