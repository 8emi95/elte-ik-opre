...
int main(int argc,char** argv){
 ...
 strcpy( myData.name, "Proba szoveg" ); 
 myData.number = 10;

 int f2;

 f2=open("testfile", O_WRONLY|O_CREAT|O_TRUNC,S_IRUSR|S_IWUSR);
  
  if (f2 < 0 ) { perror( "Error at opening file!\n " ); exit(1);  }

  write(f2, &myData, sizeof(myData )  );

  printf("Data was written out.\n");
