...
int main (int argc,char* argv[]) {
    ...
    kulcs=IPC_PRIVATE;//ftok(argv[0],1); //ftok helyett:
	//ide jöhet még IPC-PRIVATE : rejtve marad az azonosito 
	...
	// nem tud az shmget memot foglalni ha ar hasznalva van az azonosito, vagy ha nincs eleg memo
 s = shmat(oszt_mem_id,NULL,0);
    ...
    if(pid>0){    
       ...
       // beirunk a memoriaba 
       strcpy(s,buffer); //string copy, spec. memocopy -> de egy normal memocopynal a memohosszra van szukseg,
						//itt nem kell a hossz m a /0 szoveg miatt csak addig masol mig azt meg nem talalja.
