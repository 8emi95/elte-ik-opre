		...
		switch(key) {
			...
			case '8': {
				...
				if(childpid == 0) { // Child process
					...
					printf("Guestlist recieved, guests to come: \n");
					for(i = 0; i<guestNum; ++i) {
						printf("Guest #%d: %s\n", i, guests[i]);
					}
					sleep(2);
					...
					signal(SIGRTMIN, getSignal);
					
					char* location = getLine(100, "Location of the event: ");
					
					write(fileDescriptor[1], location, (strlen(location) + 1));
					
					pause();	//Wait for "I arrived" signal (SIGRTMIN)
					
					char* numberOfGuests = getLine(10, "Number of guest to arrive: ");
					
					write(fileDescriptor[1], numberOfGuests, (strlen(numberOfGuests) + 1));
					
					int guestNum = atoi(numberOfGuests);
					
					int i;
					for(i = 0; i < guestNum; ++i) {
						char* tempGuest = getLine(15, "Next guest: ");
						
						write(fileDescriptor[1], tempGuest, (strlen(tempGuest) + 1));
...
void getSignal(int signum) {
	printf("Partner arrived, waiting for guests\n");
}
