/*
 * 1. feladat:
 * KĂŠszĂ­tsen C programozĂĄsi nyelven programot,
 * amivel egy adott kĂśnyvtĂĄr (ĂŠs alkĂśnyvtĂĄrainak)
 * link (hard, soft) bejegyzĂŠseit tudjuk kezelni
 * (megnĂŠzni, mĂłdosĂ­tani, tĂśrĂślni).
 * 
 * (A kivĂĄlasztott mĹąveletet a fork- exec
 * utasĂ­tĂĄspĂĄr segĂ­tsĂŠgĂŠvel oldja meg!)
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void Konyvtar_Kiir(int argc, char** argv)
{
        ...
        if(p!=NULL)
        {
            while( (akt = readdir(p)) != NULL ) {
                ...
                if(S_ISLNK(s.st_mode)) printf("  S\t");   // symlink megjelĂślĂŠse
