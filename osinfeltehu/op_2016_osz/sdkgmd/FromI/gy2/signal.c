...
int main(){
  ...
  { 
    ...
    printf("Parent: Signal arrived, pause ended!\n");
    int status;
    printf("Parent: Send a signal back!\n");
    kill(child,SIGUSR1);
    wait(&status);
  ...
  else 
  {
    ...
    //sleep(1);
    kill(getppid(),SIGRTMIN); // SIGRTMIN eseten megy,celba er mindket jelzes
    //sleep(1);  // ha varunk, akkor a masodik signal is celba er
    pause();
    printf("Child: Parent signal arrived!");
	//kill(getppid(),SIGRTMIN);  // ha nem varunk, akkor a masodik SIGUSR1
