...
void rend_lebonyolitas(RendS* r)
{
	...
	if (0 != child)	
	{
		...
		printf("%s\n", "1 kuldtem az esemenyt");

		printf("%s\n", "1 várom a megerkezett");
		...
		printf("%d\n", num);
		write(pipe_file[1], &num, sizeof(int));
	...
	else
	{
		...
		int v_szama = 0;
		printf("%s\n", "2 olvasom v szamat");
		read(pipe_file[0], &v_szama, sizeof(int));
		printf("%s: %d\n", "2 olvastam v szamat", v_szama);


		struct Vendeg reszveveok[v_szama];
		for(int i = 0; i < v_szama; i++)
		{
			struct Vendeg v;
