...
*/    
	...
	{	
		...
		write(pipe_file[1], argv[1], strlen(argv[1]) + 1);

		wait(NULL);

		read(pipe_file[0], buffer, 300);
		printf("%s", buffer);
	}
	/* child process */
	else
	{
		...
		read(pipe_file[0], buffer, strlen(argv[1]) + 1);
		...
		write(pipe_file[1], result, strlen(result) + 1);
		/*****************************************************/
	}
