...
void rendezveny_lebonyolitas(){
	...
	char resztvevokListaja[35];
	int resztvevokSzama;

 	char hianyzoResztvevokListaja[35];
	int hianyzokSzama;
	...
	int i = 1;
                ...
                if (segedJelentkezesElem != 0) {
                    do {
                            if (isEqual(seged->rendezvenyAzonosito, segedJelentkezesElem->rendezvenyAzonosito) == 1) {
                                ++resztvevokSzama;
                                strcat(resztvevokListaja, " ");
                                strcat(resztvevokListaja, segedJelentkezesElem->nev);
                            }
                            segedJelentkezesElem = segedJelentkezesElem->next;
                    } while (segedJelentkezesElem != 0);
                }
     ...
     if(resztvevokSzama < 2){
        hianyzokSzama = 0;
     }else if(resztvevokSzama<=20){
        hianyzokSzama = 1;
     }else{
        hianyzokSzama = 2;
     }
    // printf("Hianyzok szama: %d\n", hianyzokSzama);

     struct rendezvenyElem *seged1 = rendezvenyek;
     struct rendezvenyElem *elozo1 = 0;
     int counter;
     int j = 1;
			while (seged1 != 0 && j != melyik) {
				elozo1 = seged1;
				seged1 = seged1->next;
				j++;
			}
			if (seged1 == 0) printf("Nincs ilyen sorszamu rendezveny a listaban!\n");
			else {
               struct jelentkezesElem *syy1 = jelentkezesek;

                if (syy1 != 0) {
                    do {
                            if (isEqual(seged1->rendezvenyAzonosito, syy1->rendezvenyAzonosito) == 1) {
                                strcat(hianyzoResztvevokListaja, " ");
                                strcat(hianyzoResztvevokListaja, syy1->nev);
                                ++counter;
                            }
                            syy1 = syy1->next;
                    } while (counter !=hianyzokSzama);
                }
			}
    ...
    if (pid == -1){
     ...
     if (pid == 0){// child process
        ...
        write(pipeJelentes[1], &hianyzoResztvevokListaja,sizeof(hianyzoResztvevokListaja));
        ...
        printf("Rendezveny sikeressege: %s (%d /100)\n",sikeressegSzoveg, sikeresseg);
        close(pipeSikeresseg[0]);
