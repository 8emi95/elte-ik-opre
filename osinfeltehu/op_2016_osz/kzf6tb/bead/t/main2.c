...
int handleEvent(int eventid, int fPipe, struct pollfd *poll_fds)
{
    ...
    else
    {
        ...
        // wait for event to end
        sleep(3);
        printf("[Gyerek] Vege az esemenynek. Jelentes kuldese...\n");

        // send rating
        char strRating[10];
        sprintf(strRating, "%i/%i", rand() % 10 + 1, 10);
...
int main()
{
    ...
    for ( ii = 0; ii < eventCount; ++ii) {
        printf("event %i, gc: %i\n",ii,myEvents[ii].guestCount);
        for ( jj = 0; jj < myEvents[ii].guestCount; ++jj) {
            printf("guest %i: %s\n",jj, myEvents[ii].myGuests[jj].name );
