...
int main(int argc, char** argv)
{
	...
	if (child1 == 0) //CHILD
	{
			...
			int status2;
			struct uzenet messageFromParent2; 
			status2 = msgrcv(uzenetsor, &messageFromParent2, 1024, 0, 0 ); 
			printf ("uzenet a szulotol2 : %s\n", messageFromParent2.mtext);
	...
	else //szulo
	{
		...
		int all = atoi(argv[0]);
		printf("irdat%i",all);
		int index=(rand()%all)+1;
			
		char num[80];
		sprintf(num, "%d", all);
		const struct uzenet numMessage = { 1, *num }; 
		status = msgsnd( uzenetsor, &numMessage, sizeof(numMessage.mtext), 0 );
		printf("elkuldtem a 1.szamot\n");
		
		
		char num2[80];
		sprintf(num2, "%d", index);
		const struct uzenet numMessage2 = { 1, *num2 }; 
		status = msgsnd( uzenetsor, &numMessage2, sizeof(numMessage2.mtext), 0 );
		printf("elkuldtem a2. szamot");
