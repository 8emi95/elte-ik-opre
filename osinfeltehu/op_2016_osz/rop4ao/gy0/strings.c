...
int length(char * str)// bemenete egy karaktertömb eleje, addig megy amíg nincs meg a 0
{
 int l=0;
...
int main()
{
  ...
  str2=(char*)malloc(80*sizeof(char)); //allocate new memory 80 bájt memória foglalás, típuskényszerítés, pointer
  //calloc, realloc, free - functions for allocating memory and freeing up the memory
  str2[0]='O'; //to write value character by character, olyan mint egy tömb
  str2[1]='S';
  str2[2]=0; //at the end of a string there is a 0 character!
  printf("We've given value character by character -  str2: \'%s\'\n",str2); //new content
  
  strcpy(str2,"New content by using strcpy");   //belemásolja a str2 pointerbe a szöveget
