...
{ 
	...
	// vesszuk ki a sorbol 
	 ...
	 int hanyadik = atoi(uz.mtext);
	 
     status = msgrcv(uzenetsor, &uz, 1024, 5, 0);
	 int osszesen = atoi(uz.mtext);
	 int meghatra = osszesen - hanyadik;
	 char strVannakMeg[100];
	 snprintf(strVannakMeg, 100, "%d", meghatra);
     printf("Ennyien vannak meg hatra: %s\n", strVannakMeg);

	 int ennyi = (rand() % 10) + 1;
	 if(ennyi == 1){
		printf("Be kellett vontatni.\n\n");
		const struct uzenet uzi = { 1, "Elcsuszott egy bananhejon" };
		status = msgsnd( uzenetsor, &uzi, strlen ( uzi.mtext ) + 1 , 0 );
	 }
	 else{
		printf("Sikeresen teljesítettem az üzemet.\n\n"); 
		const struct uzenet uzi = { 1, "sikeresen telejsítette az uzemet" };
		status = msgsnd( uzenetsor, &uzi, strlen ( uzi.mtext ) + 1 , 0 );		
	 }  
