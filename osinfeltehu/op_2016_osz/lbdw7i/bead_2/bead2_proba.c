...
void startEvent(const char* eventIdentifier){
    ...
    if (pid > 0)
    {
        ...
        printf("PARENT| Before sigsuspend.\n");
        //sigsuspend(&sigset);  
        //waitpid(pid, 0, 0);
        printf("PARENT| After sigsuspend\n");

        //fogadjuk a rendezvény kiértékelését, hányan vettek részt
        close(myPipe[1]);
        //open(myPipe[0]);
        int result=0;
        printf("PARENT| Before READ!\n");
        read(myPipe[0], &result, sizeof(result));
        printf("PARENT| %d guests were there!\n", result);
    ...
    else if (pid == 0)
    {
        ...
        //visszaküldjük, hogy mennyire volt sikeres
        close(myPipe[0]);
        //open(myPipe[1]);
        write(myPipe[1], &result, sizeof(result));
        //close(myPipe[1]);
        //kill(getppid(), SIGUSR1);
