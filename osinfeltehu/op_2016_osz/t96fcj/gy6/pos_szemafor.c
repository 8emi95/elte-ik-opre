...
int main (int argc,char* argv[]) {
	...
	printf("Szulo, szemafor up!\n");
       	sem_post(semid);	// semaphore up : Szülõ felengedi a semid szemafort 
	...
	if ( child == 0 ) //child
		...
		printf("Gyerek: Indul a posix szemafor down!\n");
       		sem_wait(semid);	// semaphore down - gyerek vár a szemafor elengedésére!
