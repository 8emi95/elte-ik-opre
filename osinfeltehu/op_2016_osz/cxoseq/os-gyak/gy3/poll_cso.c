...
int main(){
	...
	// 5 különboző csővezetékre van (más tipust is tud várni ezen kívűl)
	struct pollfd poll_fds[5]; // poll file descriptor array
	poll_fds[0].fd=f; 	// file decriptor
	// megadjuk mire figyeljen - itt most beérkező adatra (lehetne kimenőre.)
	// ppoll még több mindent tud
	// select is hasonlóan működik ! !! ! ! ! 
	...
	//   -- hol van az a mem cím ahol tömb van; 
	...
	printf("Second part!\n"); 
	// figyeljük h a 0-s csőbe érkezik -e adat.
	...
	{ // child process
		...
		{ 
			...
			// megkérdezzük a 0 -s csövet hogy érkezett-e adat.
			// ezt meg kell tenni mindegyikre ha több is van
