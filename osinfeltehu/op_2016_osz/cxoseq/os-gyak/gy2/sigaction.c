...
int main(){
  ...
  // itt adható meg milyen signalokra akarok reagálni.
  ...
  // mi nem haszn. - ha valamilyen jelzés csooport már be van állítva és azt itt felülírtuk már de a régit is 
  // meg akarjuk tartani akkor az itt megadható paraméterre kimenthetjuk
  ...
  if (child>0)
  {
	...
	// minden jelzést tegyük bele
    sigfillset(&sigset);
	
	// kidobáljuk hogy melyik kódok nem kellenek a sigset-be
    sigdelset(&sigset,SIGTERM);
	
	//pause() funkciója csak meg lehet adni milyen jelzéseket blokkoljon. ilyenkor a blokk feloldása automatikus
	// itt most a SIGTERM-re nem fog reagálni
    sigsuspend(&sigset);
