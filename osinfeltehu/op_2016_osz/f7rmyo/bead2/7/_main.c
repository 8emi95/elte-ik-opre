...
void parentProcess(XPipe* pipe)
{
	...
	for (;;) //event loop
	{
		...
		//XPipeWriteString(pipe, e.location); //PIPE STEP 1
		XPipeWriteAny(pipe, e.location, sizeof(e.location));
		...
		*/ 
		...
		{				
			...
			{				
				...
				//XPipeWriteString(pipe, g.name); //PIPE STEP 3
				XPipeWriteAny(pipe, g.name, sizeof(g.name));
...
void childProcess(XPipe* pipe)
{
	...
	printf("\n[ORGANIZER] debug preloop: %d\n", guestnum);
	
	for (i=0; i<guestnum; i++) //guest loop
	{
		//XPipeReadString(pipe, &str); //PIPE STEP 3
		XPipeReadAny(pipe, &buf, sizeof(buf));
		//guestlist[i] = buf;
		sprintf(guestlist[i],buf);
