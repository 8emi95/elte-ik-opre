...
void parentProcess(XPipe* pipe, XPipe* pipe2)
{
    // A parentnak most nem kell az olvasó pipe, ezért bezárjuk előre
    XPipeCloseRead(pipe);
	XPipeCloseWrite(pipe2);
	...
	for (;;) //event loop
	{
		...
		if (n < 1) { //exit when eof
			...
			printf("\n\n[COMPANY] All events have been processed! \n\n");
			XPipeWriteString(pipe, "finished"); //pipe step1, step 7
			break; 
		}	
		...
		*/ 
		...
		//we receive the rating
		XPipeReadInt(pipe2, &score); //pipe step 4
		printf("\n[COMPANY] Rating for %s:  %d/100\n",e.name,score);  
		
		//we receive the list of guests who didnt show up
		XPipeReadString(pipe2, &str); //pipe step 5,6
		while (strcmp(str.data,"continue")!=0) 
		{
			printf("\n[COMPANY] Failed to attend: %s",str.data);
			XPipeReadString(pipe2, &str); 
			sleep(1);
		}
...
void childProcess(XPipe* pipe, XPipe* pipe2)
{
	...
	XPipeReadString(pipe, &str2);  //PIPE STEP 1	   

	//while (strcmp(str.data,"finished")!=0) //event loop
	while (strcmp(str2.data,"finished")!=0) //event loop
	{
		...
		for (i=0; i<guestnum; i++) //guest loop
		{
			...
			printf("\n[ORGANIZER] %d:  %s",i+1,guestlist_str[i].data);			
		}
		
		printf("\n[ORGANIZER] The event has started! Please wait..\n");
		sleep(3);
		
		//we generate a score for the event
		r = rand()%100+1;	
		//printf("\n[ORGANIZER] Sending score: %d",r);
		
		//pipe step 4, child to parent
		XPipeWriteInt(pipe2, r);
		
		//we check if any of the guests failed to show up
		
		for (i=0; i<guestnum; i++)
		{
			r = rand()%100+1;	
			
			if (r<10)  //missing guest 10%
			{
				//XPipeWriteString(pipe2, &guestlist_str[i].data[0]); //pipe step 5
				XPipeWriteXString(pipe2, &guestlist_str[i]); //pipe step 5
				sleep(1);
			}
		}
		...
		//pipe step 6	
		// test
		strcpy(str.data, "continue");
		str.size=(sizeof("continue"));
		XPipeWriteXString(pipe2, &str); //finished with missing guests
		//endtest
		//XPipeWriteString(pipe2, "continue"); //finished with missing guests
		
		
		
		printf("\n[ORGANIZER] Event finished\n");
		
		XPipeReadString(pipe, &str2); //pipe step 1 step 7
	}
