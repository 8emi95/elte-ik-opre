...
int main(int argc,char** argv){
	...
	if (child>0) 
	{
		...
		while(safetyNet1 < 10 && event_list->head != NULL) {
			...
			read(pipefd[0], &event_result->successRate, sizeof(event_result->successRate));
			printf("Success rate: %i%%\n", event_result->successRate);
			read(pipefd[0], &event_result->numberOfMissingGuests, sizeof(event_result->numberOfMissingGuests));
			if (event_result->numberOfMissingGuests > 0) {
				...
				read(pipefd[0], event_result->missingGuests, sizeof(event_result->missingGuests));
	...
	else //child process
	{
		...
		while(hasMore) { 
				...
				while(ind < numberOfGuests) {
					...
					if(attended < 50) {
						...
						if (event_result->missingGuests == NULL) {
							event_result->missingGuests = (char*) malloc(51 * sizeof(char));
							strcpy(event_result->missingGuests, guestName);
						} else {
							event_result->missingGuests = (char*) realloc(event_result->missingGuests, (event_result->numberOfMissingGuests * 50 + 1) * sizeof(char));
			...
			write(pipefd[1], &event_result->successRate, sizeof(event_result->successRate));
			write(pipefd[1], &event_result->numberOfMissingGuests, sizeof(event_result->numberOfMissingGuests));
			if (event_result->numberOfMissingGuests > 0) {
				write(pipefd[1], event_result->missingGuests, sizeof(event_result->missingGuests));
			}
