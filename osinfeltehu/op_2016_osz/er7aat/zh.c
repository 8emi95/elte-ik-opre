...
int signalHandler(int signum, int messageQueue2)
{
	if(signum == SIGUSR1)
	{
		printf("Bevontattak\n");
...
void ParentProc(int childPid, int messageQueue, int messageQueue2, int max)
{
	...
	pause();
	struct Cause ok;
		if(receiveCause(messageQueue2, &ok) >= 0)
			printf("A szerelvenyem emiatt allt le: %s\n", ok.cause);
}
...
void ChildProc(int messageQueue1, int messageQueue2, int max)
{
	...
	else
	{
		...
		struct Cause ok1 = {1, "Beertem"};
		struct Cause ok;
		memcpy(ok, &ok1, sizeof(ok1));
		sendCause(messageQueue2, ok);
		int status = msgctl( messageQueue2, IPC_RMID, NULL );
		if ( status < 0 ) 
			perror("msgctl");
	}
}
...
int main(int argc, char* argv[])
{
	...
	{
		ParentProc(child1, messageQueue, messageQueue2, maximum);
