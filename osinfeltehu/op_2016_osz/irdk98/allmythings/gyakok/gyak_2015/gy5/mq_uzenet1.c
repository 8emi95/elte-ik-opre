...
int main (int argc, char* argv[]) { 
	...
	mq1=mq_open(mqname, O_CREAT|O_RDWR,0600,&attr);
     ...
     if ( child > 0 ) { 
	  ...
	  int db=mq_send(mq1,msg,17, 3 );  //Parent sends a message. 
	  //sleep(1); // ha ez nincs a fogad be is olvassa!!!!
          mq_send(mq1,"Hurray Operating systems",30, 4);
	  //mq_receive(mq1, buffer, strlen(buffer), 0); // get the first message
	  printf("The parent sent the message!%d\n",db);
	  wait( NULL ); 
          // After terminating child process, the message queue is deleted. 
          mq_close(mq1);
	  mq_unlink(mqname);
	  printf("The parent's end!\n"); 
     } else  { // child
	//sleep(1); 
	...
	//
	printf("Buffer length: %d\n",strlen(buffer));
	printf("mq_receive : %s, the length of read bytes: %d errno: %s\n",buffer, db,strerror(errno));
        db=mq_receive(mq1,buffer, 50, 0);
	printf("The next message: %s length: %d\n", buffer,db);
	mq_close(mq1);
