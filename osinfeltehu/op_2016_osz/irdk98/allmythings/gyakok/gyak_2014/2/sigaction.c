...
int main(){
  ...
  //during execution of handler these signals will be blocked plus the signal    
  //now only the arriving signal, SIGTERM will be blocked
  //we do have to use sigempty or sigfillset functions!!
  //we can add or subtract signals with sigdelset or sigaddset functions
  sigact.sa_flags=0; //nothing special behaviour
  sigaction(SIGTERM,&sigact,NULL);  //You can use signal function too
  ...
  if (child>0)
  {
    ...
    //after suspend, the blocked signal will arrive as well!!
    printf("The program comes back from suspending, \n Examine the order executed signal handlers!\n");
    int status;
    wait(&status);
  ...
  else 
  {
    ...
    sleep(3); //Only for making a demo we use sleep
