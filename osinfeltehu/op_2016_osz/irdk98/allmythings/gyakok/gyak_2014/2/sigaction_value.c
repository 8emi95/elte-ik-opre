...
struct data mydata={"Message via signal",111}; 
timer_t t_id; 
  ...
  switch (info->si_code){ 
    ...
    case SI_QUEUE: printf("It was sent by a sigqueue, sending process (PID %i)\n",info->si_pid);
             //    printf("Additional value: %i\n",info->si_value.sival_int);
                   struct data* d=(struct data*)info->si_value.sival_ptr;                     
...
int main(){
  ...
  sigact.sa_flags=SA_SIGINFO; //we need the siginfo 
  sigaction(SIGTERM,&sigact,NULL); 
  
  struct sigevent myevent; //sigevent for creating a timer
  myevent.sigev_signo=SIGTERM; //You should use SIGRT.. to send additianal data, but...  
