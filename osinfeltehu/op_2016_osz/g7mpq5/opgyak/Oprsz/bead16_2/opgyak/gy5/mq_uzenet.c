...
mqdes1 = mq_open(mqname, O_CREAT|O_RDWR, 0600, &attr); // letrehozza az uzenet sorunkat
sigemptyset(&set);
act.sa_flags = SA_SIGINFO; // signal info will be transferred via sigev_value
act.sa_mask = set;	// every signal is accepted
act.sa_sigaction = handler;
sigaction(SIGUSR1, &act, 0); // SIGUSR1 could be SIGRTMAX   // jelzes beallitasa, lehetne signal is

// feljelentkezes az uzenet sorra
...
mq_notify(mqdes1, &notify); // mq notification is set, so  // feljelentkezes
...
if(pid == 0) // child
	...
	// akkor is tudok dolgozni, mig nem jon uzenet???
