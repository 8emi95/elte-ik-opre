...
int szemafor_letrehozas(const char* pathname,int szemafor_ertek){
    ...
    if((semid=semget(kulcs,1,IPC_CREAT|S_IRUSR|S_IWUSR ))<0) // szemafor csaladot hozok letre, megadom, h hany tagja legyen
	perror("semget");
    // semget 2. parameter is the number of semaphores   
    if(semctl(semid,0,SETVAL,szemafor_ertek)<0)    //0= first semaphores // szemafor sorszama, inicializalo resz
        perror("semctl");
