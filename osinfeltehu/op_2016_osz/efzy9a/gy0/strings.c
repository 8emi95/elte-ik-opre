...
// left trim
// visszaadja azt a szoveget, ami balrol mar nem tartalmaz szokozt
// c-ben nincs referencia szintu parameteratadas
char* function(char* parameter){
	char* return_value = (char*)malloc(1 * sizeof(char));
	while(*parameter != ' '){++parameter;}// Stepping to the first space character
	/*-> felesleges lefoglalni meg egy pointert-> nem egy string objektum
	int i = 0;// indexing
	while(*parameter != '0'){
		return_value = (char*)realloc(i * sizeof(char));
		*return_value[i] = *parameter++;
	}*/
	return parameter;
}
int main()
{
	char* p = "adf asdfasdfasdf asdfdfggggg";
	printf("String: %s", function(p));
