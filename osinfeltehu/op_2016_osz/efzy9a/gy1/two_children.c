// egy szulo es ket gyerek folyamat letrehozasa
#include <stdio.h>
#include <stdlib.h>
...
int main()
{
 ...
 pid_t  child_second=fork();
 if (child_second<0){perror("The fork calling was not succesful\n"); exit(2);} 
 if (child>0) //the parent process, it can see the returning value of fork - the child variable!
 {
	...
	if (child_second>0) //the parent process, it can see the returning value of fork - the child variable!
 {
    waitpid(child_second,&status,0); 
    //waits the end of child process PID number=child, the returning value will be in status
    //0 means, it really waits for the end of child process - the same as wait(&status)
    printf("The value is %i in parent process (remain the original) \n",notacommonvalue);
